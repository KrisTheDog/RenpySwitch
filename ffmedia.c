...
// ИСПРАВЛЕНО: Добавлен const для совместимости с FFmpeg 7.x
...
/* Прототипы функций для работы с очередями пакетов */
...
static void deallocate(MediaState *ms) {
    ...
    /* ВАЖНО: НЕ очищаем audio_out_frame сразу - это может быть в использовании */
    
    // Очистка всех оставшихся видео поверхностей
    while (1) {
    ...
    if (ms->audio_decode_frame) {
        ...
        ms->audio_decode_frame = NULL;
    }

    /* ВАЖНО: audio_out_frame очищаем ПОСЛЕДНИМ, так как он может использоваться в audio callback */
    // НЕ ОЧИЩАЕМ ЗДЕСЬ: ms->audio_out_frame будет очищен позже
    
    ms->audio_out_index = 0;

    // Очистка всех фреймов в аудио очереди
    while (1) {
    ...
    ms->audio_queue.first = NULL;
    ms->audio_queue.last = NULL;
    ms->audio_queue_samples = 0;

    /* Очистка пакетных очередей */
    while (first_packet(&ms->video_packet_queue)) {
        dequeue_packet(&ms->video_packet_queue);
    }
    while (first_packet(&ms->audio_packet_queue)) {
        dequeue_packet(&ms->audio_packet_queue);
    }

    /* Закрытие кодеков и контекстов */
    if (ms->video_context) {
        avcodec_free_context(&ms->video_context);
        ms->video_context = NULL;
    }
    if (ms->audio_context) {
        avcodec_free_context(&ms->audio_context);
        ms->audio_context = NULL;
    }
    ...
    /* Освобождение ресурсов синхронизации */
    if (ms->cond) {
    ...
    /* ВАЖНО: audio_out_frame очищаем ТОЛЬКО здесь, если он еще не очищен */
    if (ms->audio_out_frame) {
        av_frame_free(&ms->audio_out_frame);
        ms->audio_out_frame = NULL;
    }

    /* Добавление в очередь для финального освобождения */
...
/* Выполнение части деаллокации, отложенной до основного потока. */
static void deallocate_deferred() {
    ...
    while (deallocate_queue) {
        ...
        // Ожидание завершения потока, если он есть
        if (ms->thread) {
            SDL_WaitThread(ms->thread, NULL);
            ms->thread = NULL;
        }

        // Очистка всей памяти MediaState
        av_free(ms);
    }
...
static void decode_audio(MediaState *ms) {
    ...
    while (ms->audio_queue_samples < ms->audio_queue_target_samples) {
        ...
        while (1) {
            // ВАЖНО: используем av_frame_unref вместо повторного аллока
            av_frame_unref(ms->audio_decode_frame);
            ...
            if (converted_frame == NULL) {
                // НЕ вызываем av_frame_unref здесь - фрейм еще используется
                ms->audio_finished = 1;
                return;
            }

            converted_frame->sample_rate = audio_sample_rate;
            // ИСПРАВЛЕНО: Использование нового API ch_layout для FFmpeg 7.x
            converted_frame->ch_layout = (AVChannelLayout)AV_CHANNEL_LAYOUT_STEREO;
            converted_frame->format = AV_SAMPLE_FMT_S16;

            // ИСПРАВЛЕНО: Проверка и инициализация ch_layout
            if (ms->audio_decode_frame->ch_layout.order == AV_CHANNEL_ORDER_UNSPEC) {
                ...
                if (audio_equal_mono && (ms->audio_decode_frame->ch_layout.nb_channels == 1)) {
                    // ИСПРАВЛЕНО: Использование swr_alloc_set_opts2
                    swr_alloc_set_opts2(
                        &ms->swr,
            ...
            if(swr_convert_frame(ms->swr, converted_frame, ms->audio_decode_frame)) {
                ...
                continue; // Продолжаем с следующим фреймом
            }

            // ВАЖНО: НЕ вызываем av_frame_unref здесь - фрейм будет очищен позже
            // av_frame_unref(ms->audio_decode_frame); // УБРАТЬ ЭТУ СТРОКУ
            ...
            if (start >= ms->skip) {
                ...
                // ВАЖНО: Если есть старый audio_out_frame, освобождаем его
                if (ms->audio_out_frame) {
                    av_frame_free(&ms->audio_out_frame);
                }
...
static SurfaceQueueEntry *decode_video_frame(MediaState *ms) {
    ...
    while (1) {
        ...
        if (ret == 0) {
            ...
            ms->video_finished = 1;
            av_packet_free(&pkt); // ВАЖНО: освобождаем пакет при ошибке
            return NULL;
        }
    ...
    double pts = ms->video_decode_frame->best_effort_timestamp * 
                 av_q2d(ms->ctx->streams[ms->video_stream]->time_base);

    if (pts < ms->skip) {
        // ВАЖНО: освобождаем фрейм если он до skip
        av_frame_unref(ms->video_decode_frame);
        return NULL;
    }

    // Если мы отстаем в декодировании более чем на 5 секунд, сдаемся
    if (ms->video_pts_offset && (ms->video_pts_offset + pts < ms->video_read_time - 5.0)) {
        av_frame_unref(ms->video_decode_frame); // ВАЖНО: освобождаем фрейм
        ms->video_finished = 1;
        return NULL;
    }

    // Если разрешено дропать кадры и мы опаздываем, дропаем этот кадр
    if (ms->frame_drops && ms->video_pts_offset && 
    ...
    // ВАЖНО: Освобождаем декодированный фрейм после использования
    av_frame_unref(ms->video_decode_frame);
...
int media_video_ready(struct MediaState *ms) {
    ...
    if (ms->video_pts_offset) {
        while (ms->surface_queue) {
            ...
            /* ВАЖНОЕ ИЗМЕНЕНИЕ: Освобождаем все ресурсы при удалении кадра */
            SurfaceQueueEntry *sqe = dequeue_surface(&ms->surface_queue);
            ms->surface_queue_size -= 1;
    ...
    /*
     * Иначе проверяем, есть ли кадр с PTS, который уже прошел.
     */
    if (ms->surface_queue) {
    ...
    /* Сигнализируем только если мы что-то потребили. */
    if (consumed) {
        ms->needs_decode = 1;
        SDL_CondBroadcast(ms->cond);
    }
...
static int decode_thread(void *arg) {
    ...
    if (ms->audio_duration < 0) {
        // ИСПРАВЛЕНО: Прямой доступ к полю вместо устаревшей функции
...
int media_read_audio(struct MediaState *ms, Uint8 *stream, int len) {
    ...
    while (len) {
        ...
        if (ms->audio_out_index >= f->nb_samples * BPS) {
            // ВАЖНО: Освобождаем фрейм и сбрасываем указатель
            av_frame_free(&ms->audio_out_frame);
            ms->audio_out_frame = NULL;
            ms->audio_out_index = 0;
        }
...
void media_pause(MediaState *ms, int pause) {							
    ...
    if (pause && (ms->pause_time == 0)) {
        ...
        ms->time_offset += now - ms->pause_time;
        ms->pause_time = 0;													  
    }
}
...
void media_close(MediaState *ms) {
    ...
    /* Сообщаем декодеру о завершении. Он выполнит деаллокацию всего за нас. */
    SDL_LockMutex(ms->lock);
    ms->quit = 1;
    
#ifdef __EMSCRIPTEN__
    // Для синхронного режима вызываем завершающую очистку
    media_read_sync_finish(ms);
#endif

    /* Пробуждаем поток декодера, если он ждет */
    SDL_CondBroadcast(ms->cond);
    SDL_UnlockMutex(ms->lock);
    
    // Для случая без потока (синхронный режим) выполняем очистку сразу
    if (!ms->thread) {
        deallocate(ms);
        return;
    }
    // Запускаем отложенную очистку
    deallocate_deferred();
}
...
void media_init(int rate, int status, int equal_mono) {
    ...
    if (status) {
        ...
        // Устанавливаем уровень только для ошибок
